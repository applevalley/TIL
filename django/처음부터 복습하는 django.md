...
- 파이썬 웹 프레임워크
- 표현과 로직(view)를 분리
  - 탬플릿 시스템은 '표현'을 제어하는 도구이자 표현에 관련된 로직일 뿐이라고 본다.
  - 템플릿 시스템은 이러한 기본 목표를 넘는 기능을 지원하지 말아야 한다.
- 중복 배제
  - 다수의 동적 웹 사이트는 공통적인 헤더, 푸터, 네비게이션 바같은 디자인을 가진다. Django의 템플릿 시스템은 이러한 요소들을 한 곳에저장하기 쉽게 해 중복되는 코드를 최소화해야 하고, 이것이 템플릿 상속의 기초가 된다.
...
- starts app의 view에도 index가, pages app의 view에도 index가 있을 수 있다.
  - 단순히 `{% url 'index' %}`라고만 쓰면 장고는 어느 앱의 view에 접근할까?
  - 이러한 문제를 해결하기 위해 각 app의 urls.py에 app_name을 부여해 이름 공간을 설정한다.
  - 아래와 같이 이름 공간을 설정해주면 접근이 더 명확해지고, url도 다르게 사용할 수 있다.

![image-20210112000323923](처음부터 복습하는 django.assets/image-20210112000323923.png)

![image-20210112000352677](처음부터 복습하는 django.assets/image-20210112000352677.png)

### Django Namespace

- 위에서 보듯 django에서는 서로 다른 app에서의 같은 이름을 가진 url 이름은 app_name을 설정해 구분한다.
  - templates, static에서도 django는 정해진 경로 하나로 모아서 보기 때문에 중간에 폴더를 임의로 만들어 이름 공간을 설정한다.
...
- 사이트의 모든 공통 요소를 포함하고, 하위 템플릿이 재정의 가능한 블록을 정의하는 "스켈레톤" 템플릿을 만들 수 있다.
- 이는 코드의 재사용성에 중점을 둔 것이다.

- 상속을 위한 템플릿을 만들어보자.
  - django는 위의 이름 공간에서 살펴보았듯 `app_name/templates`를 바라보기에 app이 아닌 프로젝트 폴더 내에 위치시키고자 하는 템플릿을 찾을 수 없다.
  - 그렇기에 이런 템플릿을 django가 읽게 하기 위해 django에게 위치를 알려주어야 한다.
  - settings.py의 TEMPLATES 부분의 DIRS를 프로젝트 폴더의 templates로 설정해주자.

![image-20210112002958173](처음부터 복습하는 django.assets/image-20210112002958173.png)

- 이렇게 상속해주고자 하는 템플릿 안에서는 `{% block [content] %}`과 `{% endblock %}` 태그가 있다.
  - 블록 태그는 하위 탬플릿이 채울 수 있는 공간이며 재지정할 수 있는 블록이 정의되는 곳이다.

![image-20210112003438491](처음부터 복습하는 django.assets/image-20210112003438491.png)

- 그러면 자식 탬플릿이 부모 탬플릿을 상속받으려면 어떻게 해야 할까?
  - 자식 탬플릿이 부모 탬플릿을 확장한다는 것을 선언해야 한다.
  - 이 때 `{% extends '' %}`를 사용하는데, extends 태그는 반드시 문서 최상단에 위치해야 한다.
  - 그 뒤 하위 탬플릿의 내용을 block 태그 안에 채워나간다.
  - ㅂ

![image-20210112003902324](처음부터 복습하는 django.assets/image-20210112003902324.png)

![image-20210112004207419](처음부터 복습하는 django.assets/image-20210112004207419.png)